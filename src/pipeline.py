...
def update_centroids(centroids, labels, n_features, ttl, skipped, dist_threshold = 15):
    ...
    for i,idx in enumerate(indices): #show those 22 whose tracks are positive
...
def process_frame(img, classifier, scaler, process_pool, track_centroids, veh_ttl, skipped_tracks, visualize = False, thres = 2.):
    ...
    track_centroids.append(heatmap)

    n = np.zeros((heatmap.shape[0], heatmap.shape[1], len(track_centroids)))
    for i,hm in enumerate(track_centroids):
        n[:,:, i] = hm
    sum_hm = n.sum(axis=2)
    #pdb.set_trace()
    heatmap_threshold(sum_hm)
    labels, n_features =  label(sum_hm)
...
if __name__ == '__main__':
    dat_filename = '../models/model_liblinear_ycrcb.dat'
